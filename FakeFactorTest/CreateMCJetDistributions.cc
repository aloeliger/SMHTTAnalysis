...
void CreateMCJetDistributions(string FileName)
{
  TFile* TheFile = new TFile(("/data/aloeliger/SMHTTData/"+FileName+".root").c_str());
  TTree* Tree = (TTree*) TheFile->Get("mt_tree");  
  ...
  TH1F* MCJetDistribution = new TH1F((FileName+"JetDistribution").c_str(),
				     (FileName+"JetDistribution").c_str(),
				     20,
				     50.0,
				     150.0);
	...
	if(q_1 * q_2 < 0.0 and TransverseMass < 50.0 and PZeta > -25.0 and gen_match_2 < 6.0)      
	{
	  if(byVLooseIsolationRerunMVArun2v2DBoldDMwLT_2 and !byTightIsolationRerunMVArun2v2DBoldDMwLT_2)
	    {
	      if(std::abs(Event_Fake_Factor) > 200.0)
		{
		  std::cout<<"Large fake factor!"<<std::endl;
		  std::cout<<"Event_Fake_Factor: "<<Event_Fake_Factor<<std::endl;
		}
	      if(Event_Fake_Factor < 0.0)
		{
		  std::cout<<"Negative fake factor!"<<std::endl;
		  std::cout<<"Event_Fake_Factor: "<<Event_Fake_Factor<<std::endl;
		}
	      if(std::abs(Mu_ID_SF) > 200.0)
		{
		  std::cout<<"Large Mu ID SF!"<<std::endl;
		  std::cout<<"Mu_ID_SF: "<<Mu_ID_SF<<std::endl;
		}
	      if(Mu_ID_SF < 0.0)
		{
		  std::cout<<"Negative Mu ID SF!"<<std::endl;
		  std::cout<<"Mu_ID_SF: "<<Mu_ID_SF<<std::endl;
		}
	      if(std::abs(Mu_Iso_SF) > 200.0)
		{
		  std::cout<<"Large Mu Iso SF!"<<std::endl;
		  std::cout<<"Mu_Iso_SF: "<<Mu_Iso_SF<<std::endl;
		}
	      if(Mu_Iso_SF < 0.0)
		{
		  std::cout<<"Negative Mu Iso SF!"<<std::endl;
		  std::cout<<"Mu_Iso_SF: "<<Mu_Iso_SF<<std::endl;
		}
	      if(std::abs(CrossSectionWeighting) > 200.0)
		{
		  std::cout<<"Large Cross Section Weighting!"<<std::endl;
		  std::cout<<"CrossSectionWeighting: "<<CrossSectionWeighting<<std::endl;
		}
	      if(CrossSectionWeighting < 0.0)
		{
		  std::cout<<"Negative Cross Section Weighting!"<<std::endl;
		  std::cout<<"CrossSectionWeighting: "<<CrossSectionWeighting<<std::endl;
		}
	      
	      MCJetDistribution->Fill(Var,Event_Fake_Factor*Mu_ID_SF*Mu_Iso_SF*CrossSectionWeighting);
	    }
	}
      }// end of the for loop
  TFile* OutFile = new TFile("JetFile.root","UPDATE");
  MCJetDistribution->Write();
  OutFile->Close();
}
